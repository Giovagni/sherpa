...
function statsCommand(opts) {
    ...
    if (!fs.existsSync(manifestPath)) {
        console.error('sherpa: no manifest found — run `sherpa generate` first');
        process.exit(1);
    }
    ...
    if (tokens > tokens_1.TOKEN_WARN_THRESHOLD) {
        console.warn(`  warning: exceeds recommended ${tokens_1.TOKEN_WARN_THRESHOLD.toLocaleString()} token budget`);
    }
}
